...
class DateWidget(BaseWidget):
    ...
    def update_lunar_info(self):
        try:
            ...
            lunar_day = sxtwl.fromSolar(solar_day.tm_year, solar_day.tm_mon, solar_day.tm_mday)
            yTG = lunar_day.getYearGZ(True)
            # lunar_day = self.lunar.getDayBySolar(solar_day.tm_year, solar_day.tm_mon, solar_day.tm_mday)
            self.lunar_info = "{}{}年 {}{}月{}  {}".format(
                DateWidget.GAN[yTG.tg], 
                DateWidget.ZHI[yTG.dz], 
                "闰" if lunar_day.isLunarLeap() else "", 
                DateWidget.YMC[lunar_day.getLunarMonth()-1], 
                DateWidget.RMC[lunar_day.getLunarDay()-1], 
                DateWidget.JIEQI[lunar_day.getJieQi()] if lunar_day.hasJieQi() else "无节气"
            )
